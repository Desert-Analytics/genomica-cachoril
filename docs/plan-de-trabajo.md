# Plan de trabajo

## Preguntas a resolver

> ¿Existen diferencias genéticas entre poblaciones en la misma zona geográfica?

> ¿Influye la distancia geográfica en los cambios entre especies?

## Propuestas

1. ANOVA — Javier
2. AMOVA — Juan
3. Dendrogramas — Brayan
4. Regresión lineal — Marcel
5. Bootstrapping — Isaul

## Entrega en repositorio

**Fecha límite:** domingo 17 de mayo a las 22:00

> Cada quien entregará una Jupyter Notebook o R Markdown donde describa su metodología, la desarrolle, obtenga resultados y los analice de manera reproducible.

## Workflow

1. Clonar el repositorio.
2. Desarrollar su metodología y documentarla mediante una libreta que se subirá al directorio `Desert-Analytics/genomica-cachoril/nbs`.
3. Subir los cambios a una rama del repositorio.
4. Hacer *pull request* + *merge* con la rama principal.

## Presentación — Equipo

**Fecha límite:** martes 19 de mayo a las 22:00

* Google Slides: [https://docs.google.com/presentation/d/1PK5heKNZk8hVjaU5NcMh3oSBVyaJQyQIB7d5JCGPtSY/edit?usp=sharing](https://docs.google.com/presentation/d/1PK5heKNZk8hVjaU5NcMh3oSBVyaJQyQIB7d5JCGPtSY/edit?usp=sharing)

### Estructura

1. Introducción
   1. Descripción del problema
   1. Objetivo del análisis
   1. Descripción de los datos

2. Desarrollo
   1. Metodología y resultados
      1.  ANOVA
      2. AMOVA
      3. Dendrogramas
      4. Regresión lineal
      5. Bootstrapping

3. Conclusiones

## Pendientes

1. [X] Formato LICE (sábado 23/mayo, 10:00)

2. [X] Datos - ETL (sábado 16/mayo, 22:00)

3. [ ] Trabajo individual (lunes 18/mayo, 22:00)

   * [x] ANOVA — Javier
   * [x] AMOVA — Juan
   * [X] Dendrogramas — Brayan
   * [x] Regresión lineal / PCA + distancias — Marcel
   * [x] Bootstrapping — Isaul

4. [X] Presentación Google Slides (martes 19/mayo, 22:00)
   [https://docs.google.com/presentation/d/1PK5heKNZk8hVjaU5NcMh3oSBVyaJQyQIB7d5JCGPtSY/edit?usp=sharing](https://docs.google.com/presentation/d/1PK5heKNZk8hVjaU5NcMh3oSBVyaJQyQIB7d5JCGPtSY/edit?usp=sharing)
